...
export const liveList = (state = initialState, action) => {
	let curPage, totalPage, totalCount, items, id, type;
	switch (action.type) {
		case types.LIVE_LIST_LIST_GET + "_ON":
			state = {
				...state,
				isEnd: 1
			};
		...
		case types.LIVE_LIST_LIST_GET + "_SUCCESS":
			...
			return state;
		case types.LIVE_LIST_LIST_GET + "_SETPAGE":
			state = {
				...state,
			...
			return state;
		case types.LIVE_LIST_LIST_GET + "_ERROR":
			state = {
				...state,
				isEnd: 3
			};
			return state;
		case types.LIVE_LIST_UPDATE_SORT_POST + "_SUCCESS":
		case types.LIVE_LIST_ROOM_DEL_POST + "_SUCCESS":
			state = {
				...initialState
			};
			return state;
		case types.LIVE_LIVE_SEARCH_INIT:
			state = {
				...initialState
			};
			return state;
