...
export const materialMain = (state = initialState, action) => {
	let currentPage, totalPage, items;
	switch (action.type) {
		case types.MATERIAL_MAIN_LIST_GET + '_ON':
			state = {
				...state,
				isEnd: 1,
				isFetching: 1
			...
			return state;
		// 上拉加载
		case types.MATERIAL_MAIN_LIST_GET + '_SUCCESS':
			...
			return state;
		// 下拉刷新
		case types.MATERIAL_MAIN_LIST_GET + '_REFRESH':
			currentPage = 1;
			totalPage = action.data.totalPage;
			items = initItem(action.data.list, 'category_id');
			state = {
			...
			return state;
		case types.MATERIAL_MAIN_LIST_GET + '_ERROR':
			state = {
				...state,
				isEnd: 3
			};
