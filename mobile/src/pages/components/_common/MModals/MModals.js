	...
	render() {
		...
		return (
			<div className={`rcm-modal ${className}`} style={{ ...style, /* ...hackForiOS11.absolute */}}>
				<div className="__bg __bg_enter" onClick={e => maskClosable && this.handleClose(e, 1)} onTouchMove={e => e.preventDefault()} ref="bg"/>
				<div className="__fixed __enter" ref="fixed">
						...
						<div className="__select">
							{
								selectData.map((item = {}, index) => {
									...
									return (
										...
										>
											<div>{item.label}</div>
											{ is_select && <i className="iconfont icon-completed" style={{ fontSize: '18px' }}/>}
										</div>
									);
								})
							}
					...
					{ // 1个按钮
						(actions.length > 0 && actions.length == 1 && type !== "operation") &&
							<div className="__flex __jc-c __bt">
								<div className="__1of2 __flex __jc-c" onClick={(e) => { this.handlePress(e, 0); }}>
									<div 
										className="__btn __blue" 
					...
					{ // 2个按钮
						(actions.length > 0 && actions.length == 2 && type !== "operation") &&
							<div className="__flex __jc-c __bt">
								<div className="__col __flex __jc-c" onClick={(e) => { this.handlePress(e, 0); }}>
									<div 
										className="__btn" 
										style={{ ...actions[0].style || {} }}
									>{actions[0].text || "取消"}</div>
								</div>
								<div className="__col __flex __jc-c __bl">
									<div 
										className="__btn __blue" 
